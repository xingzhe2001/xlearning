...
def test_fit():
      ...
      our_rg = us.LinearRegression(normalize=False, alpha=0.3)
...
def test_params():
      ...
      for alpha in alphas:
            our_rg = us.LinearRegression(normalize = False, alpha = alpha)
            our_rg.fit(X1_train, Y1_train)
            our_Y1_pred = our_rg.predict(X1_test)
      ...
      for alpha in alphas:
            our_rg = us.LinearRegression(normalize = True, alpha = alpha)
            our_rg.fit(X1_train, Y1_train)
            our_Y1_pred = our_rg.predict(X1_test)
